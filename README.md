...
This project is a small pipeline dashboard built around [Open-Meteo](https://open-meteo.com) that pulls weather data (Temperature, Percipitation, Wind Speed, Humidity) for any city you type in, stores every pull as a timestamped snapshot in a hosted [Turso](https://turso.tech) (libSQL) database, computes metrics between snapshots from the accumulated history, and asks an open-source LLM via [OpenRouter](https://openrouter.ai) to suggest a travel itinerary based on the current forecast for the selected city.

## Stack

Python 3.10, FastAPI, Turso via `libsql` (falls back to a local SQLite file via stdlib `sqlite3`), HTML/JS, Chart.js for the dashboard graph, pytest for tests.

## Environment Variables

The app reads its settings from environment variables. On my machine they are saved in a `.env` file in the project root, which is **not committed to GitHub**, and the same values are set as environment variables in the Vercel project for the deployed site.

| Variable | What it does | Default if unset |
|---|---|---|
| `TURSO_DATABASE_URL` | URL of the hosted Turso database. When set, every read and write goes to Turso and nothing is written to a local file | None (falls back to local SQLite) |
| `TURSO_AUTH_TOKEN` | Auth token that grants access to the Turso database | None |
| `OPENROUTER_API_KEY` | API key for OpenRouter, used by the itinerary feature to call the LLM | None (itinerary returns a 503 error) |
| `OPENROUTER_MODEL` | Which OpenRouter model generates the itinerary | `inclusionai/ling-3.0-flash-sante:free` |
| `DB_PATH` | Path of the local SQLite file, only used when `TURSO_DATABASE_URL` is not set | `weather.db` (`/tmp/weather.db` on Vercel) |
| `ROLLING_WINDOW` | How many recent pulls the rolling average covers | `5` |
| `ALERT_THRESHOLD_C` | Temperature change (°C) since the last pull that triggers the alert flag | `5` |

**If you clone this repo onto another machine**, you won't have my `.env` file, so:
- The **itinerary feature will not work**, because there is no `OPENROUTER_API_KEY`
- You **cannot read from or write to the Turso database**, because there is no `TURSO_DATABASE_URL` or `TURSO_AUTH_TOKEN`. The app will instead create and use an empty local `weather.db` file, so none of the cities or history stored in Turso will show up

Everything else (pulling weather, metrics, the chart) works without any environment variables. To use every feature, use the deployed site below, or create your own `.env` with your own OpenRouter key and Turso database.

## Vercel Deployment

https://nuel-assessment.vercel.app/
...
- A scheduled pull: a systemd user timer on my machine runs `scripts/pull_now.py` every 15 minutes (while my machine is on), re-pulling every tracked city and writing the new snapshots to the Turso database, so the deployed site shows them without a redeploy (see Scheduled Pull below)

## Scheduled Pull

The scheduled pull uses a systemd user timer (`weather-pull.timer`, saved to my machine)

**This timer is set up only on my own machine** it is not part of this repo and does not run on Vercel. It only runs while my machine is on and I'm logged in. Because it reads the same `.env` as the app, its pulls are written straight to the hosted Turso database, so the deployed site on Vercel shows them immediately without a redeploy. If you clone this repo, no scheduled pull will run unless you set up your own timer.
...
- Each pull inserts one new row into the `snapshots` table (temperature, windspeed, humidity, precipitation, weathercode, is_day, the daily and hourly forecast JSON, and the pull's timestamp), tagged `source='live'`.
...
```bash
systemctl --user status weather-pull.timer
```

**View logs from each run:**

```bash
journalctl --user -u weather-pull.service
```

**Change the interval:** edit `OnUnitActiveSec` in the timer file, then
...
```bash
systemctl --user disable --now weather-pull.timer
```
...
- Returns early if no city has been selected yet
...
The app stores everything in a hosted Turso (libSQL) database. The schema is:
...
29 tests, all offline, covering the derived-metric math, the pull pipeline's success/failure paths, the API endpoints end to end, and the OpenRouter integration's error handling.
...
- `config.py`: Reads settings from .env (TURSO_DATABASE_URL, TURSO_AUTH_TOKEN, DB_PATH, ROLLING_WINDOW, ALERT_THRESHOLD_C)
- `db.py`: Opens a connection to Turso (or a local SQLite file if Turso isn't configured) and creates the tables (cities, snapshots, saved_itineraries) if not created already
...
**`weather.db`**: Local SQLite database file, only created and used when TURSO_DATABASE_URL is not set (tests use their own temporary database files)
...
- App setup
  - `lifespan()` runs db_module.init_db() (init_db function in db.py) on startup to execute the schema defined in db.py, ensuring that the database tables were created
...
- Routes
  ...
  - `POST /api/pull`: Passes off two arguments to pull_city() (pull.py), conn, the open database connection, so pull_city() can read/write the database itself, city the raw city name string the user typed in
...
- Imports sqlite3
- Imports DB_PATH, TURSO_DATABASE_URL and TURSO_AUTH_TOKEN from config.py (defined in .env)
- Defines SCHEMA
  - Creates cities, snapshots and saved_itineraries tables
- `_Row`, `_TursoCursor`, `_TursoConnection`: Small wrappers around the libsql connection so Turso rows behave like sqlite3.Row (row["col"], row[0], dict(row)) and the rest of the app doesn't need to know which database it's talking to
- `get_connection()`: Opens a new connection and sets it up
  - If no db_path is passed and TURSO_DATABASE_URL is set, connects to Turso with libsql (remote only, no local copy is written)
  - Otherwise opens the local SQLite file (db_path, or DB_PATH from config.py), which is what tests and the fallback use
- `init_db()`
...
- Defines TURSO_DATABASE_URL and TURSO_AUTH_TOKEN: the hosted Turso database the app uses when the URL is set
- Defines DB_PATH: the local SQLite fallback file path, read from the DB_PATH environment variable, defaulting to "/tmp/weather.db" on Vercel or "weather.db" otherwise
...
- These values are imported wherever a setting is needed (the Turso settings and DB_PATH in db.py; ROLLING_WINDOW and ALERT_THRESHOLD_C in main.py) instead of each module reading os.environ directly
